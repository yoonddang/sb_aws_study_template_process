package com.template.process.jobstore;
...
import static com.hanteo.batch.jobstore.TriggerState.*;
import static com.hanteo.batch.jobstore.TriggerWrapper.newTriggerWrapper;

public class HazelcastJobStore implements JobStore, Serializable {

	private static final Logger logger = LoggerFactory.getLogger(HazelcastJobStore.class);
	...
	private final String HC_JOB_STORE_MAP_JOB = "launcher-store-map-launcher";
	private final String HC_JOB_STORE_MAP_JOB_BY_GROUP_MAP = "launcher-store-map-launcher-by-group-map";
	private final String HC_JOB_STORE_TRIGGER_BY_KEY_MAP = "launcher-store-trigger-by-key-map";
	private final String HC_JOB_STORE_TRIGGER_KEY_BY_GROUP_MAP = "launcher-trigger-key-by-group-map";
	private final String HC_JOB_STORE_PAUSED_TRIGGER_GROUPS = "launcher-paused-trigger-groups";
	private final String HC_JOB_STORE_PAUSED_JOB_GROUPS = "launcher-paused-launcher-groups";
	private final String HC_JOB_CALENDAR_MAP = "launcher-calendar-map";
	...
	public void initialize(ClassLoadHelper loadHelper, SchedulerSignaler signaler) throws SchedulerConfigException {
		...
		logger.debug("Initializing Hazelcast Job Store..");

		this.schedSignaler = signaler;

		if (hazelcastClient == null) {
			logger.warn("Searching hazelcast instance since not hazelcast instance setted before starting scheduler. Instance's name is '" + instanceName+ "'");
			hazelcastClient = Hazelcast.getHazelcastInstanceByName(instanceName);
			if(hazelcastClient == null) {
				logger.warn("Not found hazelcastInstanc. Starting new local hazelcast client");
				hazelcastClient = Hazelcast.newHazelcastInstance();
			}
		}

		// initializing hazelcast maps
		logger.debug("Initializing hazelcast maps...");
		...
		logger.debug("Hazelcast Job Store Initialized.");
	}
	...
	public void schedulerStarted()
		...
		logger.info("Hazelcast Job Store started successfully");
	...
	public void storeJob(final JobDetail job, boolean replaceExisting)
		...
		try {
			...
			try {
				...
				logger.warn("Error unlocking since it is already released.", ex);
			}
		}
	}
		...
		if (jobsByKey.containsKey(jobKey)) {
			...
			for (final OperableTrigger trigger : triggersForJob) {
				if (!removeTrigger(trigger.getKey(), false)) {
					logger.warn("Error deleting trigger [{}] of launcher [{}] .", trigger, jobKey);
					return false;
				}
			}
	...
	public void storeTrigger(OperableTrigger trigger, boolean replaceExisting)
		...
		try {
			...
			if (retrieveJob(newTrigger.getJobKey()) == null) {
				throw new JobPersistenceException("The launcher (" + newTrigger.getJobKey()
	...
	public Set<TriggerKey> getTriggerKeys(GroupMatcher<TriggerKey> matcher)
		...
		return outList == null
				? Collections.<TriggerKey> emptySet()
				: outList;
	}
	...
	public List<String> getJobGroupNames()
			throws JobPersistenceException {
		if(jobsByGroup == null || jobsByGroup.keySet() == null) {
			return Collections.EMPTY_LIST;
		}
		return new LinkedList<>(jobsByGroup.keySet());
	}
	...
	public void resetTriggerFromErrorState(TriggerKey triggerKey) throws JobPersistenceException {
		...
		try {
			...
			if(pausedTriggerGroups.contains(triggerKey.getGroup())) {
				...
				tw = newTriggerWrapper(tw, PAUSED);
	...
	/**
	 * Get the amount of time (in ms) to wait when accessing this launcher store
	 * repeatedly fails.
	 * <p>
	 ...
	 * @see JobStore#acquireNextTriggers(long, int, long)
	 *
	 * @param noLaterThan
		...
		for (int i = 0; i < orderedTriggers.size(); i++) {
			...
			try {
				...
				if (tw.getState() == ACQUIRED && (tw.getAcquiredAt() == null
					...
					logger.warn("Found a lost trigger [{}] that should be released at [{}]", tw, limit);
				...
				if (applyMisfire(tw)) {
					logger.debug("Misfire applied {}", tw);
					if (tw.trigger.getNextFireTime() != null) {
				...
				// If trigger's launcher is set as @DisallowConcurrentExecution, and it has
			...
			if (triggerInstCode == CompletedExecutionInstruction.DELETE_TRIGGER) {
				...
				try {
					if (trigger.getNextFireTime() == null) {
						// double check for possible reschedule within launcher
					...
					logger.error("Error removing trigger", ex);
				}
			} else if (triggerInstCode == CompletedExecutionInstruction.SET_TRIGGER_COMPLETE) {
				storeTriggerWrapper(newTriggerWrapper(tw, STATE_COMPLETED));
				schedSignaler.signalSchedulingChange(0L);
			} else if (triggerInstCode == CompletedExecutionInstruction.SET_TRIGGER_ERROR) {
				logger.warn("Trigger " + trigger.getKey() + " set to ERROR state.");
...
/**
 * Filter triggers with a given launcher key
 */
